...
module.exports = async (req, res) => {
  ...
  const updates = { activity, updated_at: new Date().toISOString(), ...(body.progress !== undefined && { progress: Number(body.progress) }), ...(body.status && { status: body.status }), ...(body.lane && { lane: body.lane }) };
  const r2 = await supabaseFetch(`/tasks?id=eq.${id}`, { method: "PATCH", body: JSON.stringify(updates) });
  await supabaseFetch("/events", { method: "POST", body: JSON.stringify({ level: body.level || "info", source, message, target: task.title, created_at: new Date().toISOString() }) });
