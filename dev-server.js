...
const seedState = {
  agents: [
    { id: "scout", name: "Scout", role: "Research", status: "Connected", model: "OpenAI GPT-5.5", lastSeen: new Date().toISOString(), capabilities: ["research", "summarize"] },
    { id: "analyst", name: "Analyst", role: "Analysis", status: "Connected", model: "Claude Opus 4.6", lastSeen: new Date().toISOString(), capabilities: ["analysis", "reports"] },
    { id: "builder", name: "Builder", role: "Frontend", status: "Connected", model: "OpenAI GPT-5.5 Codex", lastSeen: new Date().toISOString(), capabilities: ["code", "ui"] },
    { id: "guardian", name: "Guardian", role: "Run Health", status: "Run Health", model: "OpenAI GPT-5.4", lastSeen: new Date().toISOString(), capabilities: ["monitoring", "security"] },
    { id: "operator", name: "Operator", role: "Ops", status: "Connected", model: "OpenAI GPT-5.5", lastSeen: new Date().toISOString(), capabilities: ["deploy", "routing"] },
    { id: "scribe", name: "Scribe", role: "Documentation", status: "Connected", model: "Claude Sonnet 4.5", lastSeen: new Date().toISOString(), capabilities: ["docs", "memory"] }
  ],
  tasks: [
    createTaskSeed("m1", "plan", "Q2 Market Analysis", "Analyze market trends and competitor positioning.", "High", "2026-05-23", "Planning", 30, ["scout", "analyst"]),
    createTaskSeed("m2", "plan", "Customer Feedback Loop", "Aggregate and summarize customer feedback.", "Medium", "2026-05-24", "Planning", 18, ["scribe"]),
    createTaskSeed("m3", "plan", "Infra Cost Optimization", "Identify and reduce cloud spend.", "Low", "2026-05-25", "Planning", 12, ["operator"]),
    createTaskSeed("m4", "execute", "Website Redesign", "Redesign the marketing website and improve performance.", "High", "2026-05-22", "Executing", 65, ["builder", "guardian"]),
    createTaskSeed("m5", "execute", "Data Pipeline Migration", "Migrate ETL pipeline to the new platform.", "Medium", "2026-05-27", "Executing", 48, ["operator", "analyst"]),
    createTaskSeed("m6", "review", "Security Audit", "Conduct a full security audit and remediation plan.", "High", "2026-05-26", "Review", 80, ["guardian"]),
    createTaskSeed("m7", "review", "Onboarding Flow Update", "Improve onboarding flow and user activation.", "Medium", "2026-05-28", "Review", 72, ["scout", "scribe"]),
    createTaskSeed("m8", "done", "API Docs Overhaul", "Restructure and modernize API documentation.", "Medium", "2026-05-20", "Done", 100, ["scribe", "builder"]),
    createTaskSeed("m9", "done", "Release v1.4.0", "Shipped v1.4.0 to production successfully.", "High", "2026-05-19", "Done", 100, ["operator"]),
    createTaskSeed("m10", "done", "Bug Bash Sprint", "Resolved critical bugs from latest release.", "Low", "2026-05-18", "Done", 100, ["guardian", "builder"])
  ],
  events: [
    createEvent("ok", "Task", "Website Redesign moved to In Progress", "Cece"),
  ...
  scraperItems: [
    {
      ...
      score: "HIGH",
      pain: "Contractors are losing jobs because estimates take too long to prepare.",
      suggestion: "Use this as a QuoteCore+ content angle around quote speed.",
      quotes: ["I spend nights catching up on estimates."],
      draftComments: ["This is exactly the hidden cost of estimating. The job is often won or lost in the delay after the site visit."],
      tags: ["quoting pain", "comment ready"],
...
async function handleApi(request, response, pathname) {
  ...
  if (request.method === "GET" && pathname === "/api/scraper") {
    sendJson(response, 200, { items: state.scraperItems || [] });
    return;
  }

  if (request.method === "POST" && pathname === "/api/scraper") {
    ...
    const item = {
      ...
      platform: body.platform || body.sourceName || "Web Search",
      title: body.title || "Untitled scrape",
      url: body.url || body.link || "",
      score: String(body.score || body.opportunity_score || "MED").toUpperCase(),
      pain: body.pain || body.pain_point || body.summary || "",
      suggestion: body.suggestion || body.suggested_action || body.action || "",
      quotes: Array.isArray(body.quotes) ? body.quotes : [],
      draftComments: Array.isArray(body.draftComments) ? body.draftComments : Array.isArray(body.draft_comments) ? body.draft_comments : [],
      tags: Array.isArray(body.tags) ? body.tags : [],
      capturedAt: body.capturedAt || body.captured_at || new Date().toISOString()
    };
    state.scraperItems = [item, ...(state.scraperItems || [])].slice(0, 200);
    state.events.unshift(createEvent("info", item.platform, `Scraped opportunity: ${item.title}`, item.score));
    writeState(state);
    sendJson(response, 201, { item });
    return;
  }

  const scraperMatch = pathname.match(/^\/api\/scraper\/([^/]+)$/);
  if (scraperMatch && request.method === "DELETE") {
    if (!isAgentAuthorized(request)) {
      sendError(response, 401, "Agent token required");
      return;
    }
    state.scraperItems = (state.scraperItems || []).filter((item) => item.id !== scraperMatch[1]);
    writeState(state);
  ...
  if (taskMatch && request.method === "DELETE") {
    ...
    state.events.unshift(createEvent("warn", "Task", `${task.title} deleted`, "Cece"));
    writeState(state);
  ...
  if (taskMatch && request.method === "PATCH") {
    ...
    const body = await readBody(request);
    Object.assign(task, pick(body, ["lane", "title", "description", "projectId", "priority", "due", "status", "progress", "agentIds", "activity"]));
  ...
  if (request.method === "POST" && pathname === "/api/agents/register") {
    ...
    Object.assign(agent, {
      ...
      model: body.model || agent.model || "Model not reported",
